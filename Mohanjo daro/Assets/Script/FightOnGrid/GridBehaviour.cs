...
public class GridBehaviour : MonoBehaviour
{
    ...
    void GenerateGrid()
    {
        for(int i=0;i<columns;i++)
        {
            ...
            {
                GameObject obj = Instantiate(gridPrefeb, new Vector3(leftBottomLocation.x + scale * i,leftBottomLocation.y + scale * j, leftBottomLocation.z ),Quaternion.identity);
    ...
    void setDistance()
    {
        ...
        for(int step=1;step<rows*columns;step++)
        {
            foreach(GameObject obj in gridArray)
            {
                if (obj && obj.GetComponent<GridStat>().visited == -1)
                    testForDirection(obj.GetComponent<GridStat>().x, obj.GetComponent<GridStat>().y, step);
               
            }
        }
    }

    void testForDirection(int x,int y,int step)
    {
    ...
    {
        float currentDistance = scale * rows * columns;int indexNumber = 0;
