...
public class NewDialogueManager : MonoBehaviour
{
    ...
    IEnumerator TypeSentence(string sentence)
    {
        ...
        dialogueText.text = "";
        int index = sentence.ToString().IndexOf(":");
        nameText = sentence.ToString().Substring(0, index);
        ...
        //cheking the sentance length for suitable to fit in one line or not 

        if(sentence.Length>100)
        {
            string Running_Dailogue = sentence.Substring(0, 100);
            foreach (char letter in sentence.ToCharArray())
            {
                stop_Next = true;
                if (Input.GetMouseButtonDown(1))
                {
                    dialogueText.text = Running_Dailogue;
                    sentence = sentence.Substring(100, sentence.Length - 100);
                    sentence = "";
                    stop_Next = false;
                    goto Exit;
                }
